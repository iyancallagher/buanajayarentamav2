...
if (!empty($searchValue)) {
    // FIX: Mengubah u.username menjadi u.nama agar tidak memicu Fatal Error database
    $countSql .= " AND (m.type_unit LIKE ? OR m.nopol LIKE ? OR m.mekanik LIKE ? OR u.nama LIKE ?)";
...
if (!empty($searchValue)) {
    // FIX: Mengubah u.username menjadi u.nama di kueri utama juga
    $sql .= " AND (m.type_unit LIKE ? OR m.nopol LIKE ? OR m.mekanik LIKE ? OR u.nama LIKE ?)";
    $likeValue = '%' . $searchValue . '%';
    $params[] = $likeValue;
    $params[] = $likeValue;
    $params[] = $likeValue;
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $rows[] = [
        'no'         => $no++, // Diteruskan agar komponen tabel generic membaca nomor halaman yang pas
        'tanggal'    => $htmlTanggal,
