...
// Ambil data komponen untuk dropdown select
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200">
            <div class="p-6 border-b border-slate-200">
                ...
                <p class="text-sm text-slate-500 mt-1">
                    Lengkapi data di bawah untuk menambahkan master data sparepart baru.
                </p>
            </div>

            <form action="store-sparepart.php" method="POST" class="p-6 space-y-5">
                    ...
                    <div class="relative">
                        <select name="komponen_id" required
                            ...
                            <?php while ($komponen = mysqli_fetch_assoc($komponenResult)): ?>
                                <option value="<?= $komponen['id'] ?>" <?= ($_SESSION['old_input']['komponen_id'] ?? '') == $komponen['id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars('[' . $komponen['kode_komponen'] . '] ' . $komponen['nama_komponen']) ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                        <i
                            class="ti ti-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"></i>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nama_sparepart"
                            value="<?= htmlspecialchars($_SESSION['old_input']['nama_sparepart'] ?? '') ?>"
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400 mb-2">Gunakan tombol tambah jika item memiliki lebih dari satu
                        nomor part.</p>
                    <div class="space-y-2">
                        <template x-for="(part, index) in numberParts" :key="index">
                            <div class="flex gap-2">
                                <div class="relative flex-1">
                                    <i
                                        class="ti ti-hash absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-base"></i>
                    ...
                    <div class="space-y-2">
                        <template x-for="(unit, index) in typeUnits" :key="index">
                            <div class="flex gap-2">
                                <div class="relative flex-1">
                                    <i
                                        class="ti ti-truck absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-base"></i>
                                ...
                                <button type="button" @click="removeTypeUnit(index)" x-show="typeUnits.length > 1"
                                    class="p-2.5 rounded-xl border border-red-200 text-red-500 hover:bg-red-50 transition-colors">
                                    <i class="ti ti-trash text-lg"></i>
                                </button>
                            </div>
                        </template>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-600 leading-relaxed mb-3">
                            Sistem akan membuatkan urutan <code>kode_sparepart</code> secara otomatis berdasarkan
                            komponen induk terpilih.
                        </p>
                    </div>
                </div>
            </div>
